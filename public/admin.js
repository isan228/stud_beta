...
function setupAdminEventListeners() {
    ...
    const uploadUsmleTxtMixedBtn = document.getElementById('uploadUsmleTxtMixedBtn');
    if (uploadUsmleTxtMixedBtn) {
        uploadUsmleTxtMixedBtn.addEventListener('click', () => openUsmleTxtMixedUploadModal());
    }
    ...
    const addUniFlashcardWithImagesBtn = document.getElementById('addUniFlashcardWithImagesBtn');
    if (addUniFlashcardWithImagesBtn) {
        addUniFlashcardWithImagesBtn.addEventListener('click', () => openAddUniFlashcardWithImagesModal());
    }
    const uploadUniTxtFlashcardsBtn = document.getElementById('uploadUniTxtFlashcardsBtn');
    if (uploadUniTxtFlashcardsBtn) {
        uploadUniTxtFlashcardsBtn.addEventListener('click', () => openUniTxtFlashcardsUploadModal());
    }
    const uniTxtFlashcardsUploadForm = document.getElementById('uniTxtFlashcardsUploadForm');
    if (uniTxtFlashcardsUploadForm && !uniTxtFlashcardsUploadForm.dataset.bound) {
        uniTxtFlashcardsUploadForm.dataset.bound = '1';
        uniTxtFlashcardsUploadForm.addEventListener('submit', handleUniTxtFlashcardsUpload);
    }
    ...
    if (uniFcUniversityFilter) {
        uniFcUniversityFilter.addEventListener('change', async () => {
            const uni = uniFcUniversityFilter.value;
    ...
    const fcImgUniversityId = document.getElementById('fcImgUniversityId');
    if (fcImgUniversityId) {
        fcImgUniversityId.addEventListener('change', () => {
            fillUniFcTopics(fcImgUniversityId.value, '', 'fcImgTopicId');
        });
    }
    const uniTxtFcUniversityId = document.getElementById('uniTxtFcUniversityId');
    if (uniTxtFcUniversityId) {
        uniTxtFcUniversityId.addEventListener('change', () => {
            fillUniFcTopics(uniTxtFcUniversityId.value, '', 'uniTxtFcTopicId');
        });
    }
    ...
    if (usmleSaUploadBlockBtn) {
        usmleSaUploadBlockBtn.addEventListener('click', () => openUsmleSaBlockUpload(usmleSaActiveBlockIndex));
    }
    const txtSaBlockUploadForm = document.getElementById('txtSaBlockUploadForm');
    if (txtSaBlockUploadForm) {
        txtSaBlockUploadForm.addEventListener('submit', handleTxtSaBlockUpload);
    }
    const questionTestIdSel = document.getElementById('questionTestId');
    if (questionTestIdSel && !questionTestIdSel.dataset.boundSaBlock) {
        questionTestIdSel.dataset.boundSaBlock = '1';
        questionTestIdSel.addEventListener('change', syncQuestionSaBlockField);
    }
    ...
    const txtMixedUploadForm = document.getElementById('txtMixedUploadForm');
    if (txtMixedUploadForm) {
        txtMixedUploadForm.addEventListener('submit', handleTxtMixedUpload);
    }
...
async function loadUsmleQuestionsAdmin() {
    ...
    try {
        ...
        list.innerHTML = questions.map(question => renderUsmleQuestionAdminItem(question)).join('');
    } catch (e) {
...
async function loadUsmleSaBlocksAdmin(testId) {
    ...
    usmleSaActiveBlockIndex = null;
    if (blockQPanel) blockQPanel.style.display = 'none';
    if (blocksPanel) blocksPanel.style.display = '';
    if (list) {
        list.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 1rem;">Выберите блок, чтобы редактировать вопросы или загрузить TXT.</p>';
    }
    if (!body) return;

    body.innerHTML = '<tr><td colspan="5" style="padding:1rem; text-align:center; color:var(--text-muted);">Загрузка…</td></tr>';
    try {
        const res = await fetch(`${ADMIN_API_URL}/tests/${testId}/self-assessment/blocks`, {
            headers: { Authorization: `Bearer ${currentAdminToken}` }
        });
            ...
            return `<tr style="border-top:1px solid var(--border-light);">
                ...
                <td style="padding:0.85rem 1rem;">
                    <div style="display:flex; gap:0.4rem; flex-wrap:wrap;">
                        <button type="button" class="btn btn-secondary btn-sm" onclick="openUsmleSaBlock(${b.blockIndex})">Редактировать</button>
                        <button type="button" class="btn btn-primary btn-sm" onclick="openUsmleSaBlockUpload(${b.blockIndex})">TXT</button>
                        <button type="button" class="btn btn-danger btn-sm" onclick="clearUsmleSaBlock(${b.blockIndex})">Очистить</button>
                    </div>
                </td>
            </tr>`;
        }).join('');
    } catch (e) {
        body.innerHTML = `<tr><td colspan="5" style="padding:1rem;color:var(--danger-color);">${escapeAdminHtml(e.message || 'Ошибка')}</td></tr>`;
    }
}
...
async function loadUsmleSaBlockQuestions(testId, blockIndex) {
    ...
    try {
        ...
        const questions = await response.json();
        if (!questions.length) {
            list.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 1.5rem;">В блоке пока нет вопросов — загрузите TXT или добавьте вручную</p>';
            return;
        }
        ...
        list.innerHTML = '<p style="color: var(--danger-color);">Ошибка загрузки вопросов блока</p>';
    }
}
...
function openUsmleSaBlockUpload(blockIndex) {
    ...
    if (!idx) {
        showNotification('Выберите блок', 'error');
        return;
    }
    const title = document.getElementById('txtSaBlockUploadTitle');
    if (title) title.textContent = `📄 Self-Assessment: TXT → Block #${idx}`;
    const form = document.getElementById('txtSaBlockUploadForm');
    if (form) form.reset();
    document.getElementById('txtSaBlockTestId').value = testId;
    document.getElementById('txtSaBlockIndex').value = String(idx);
    const replace = document.getElementById('txtSaBlockReplace');
    if (replace) replace.checked = true;
    const modal = document.getElementById('txtSaBlockUploadModal');
...
async function clearUsmleSaBlock(blockIndex) {
    ...
    try {
        const res = await fetch(`${ADMIN_API_URL}/tests/${testId}/self-assessment/blocks/${blockIndex}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${currentAdminToken}` }
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data.error || 'Ошибка');
        showNotification(data.message || 'Блок очищен', 'success');
        usmleSaActiveBlockIndex = null;
        await loadUsmleSaBlocksAdmin(testId);
    } catch (e) {
        showNotification(e.message || 'Ошибка очистки', 'error');
    }
}

async function handleTxtSaBlockUpload(e) {
    e.preventDefault();
    const testId = document.getElementById('txtSaBlockTestId')?.value;
    const blockIndex = document.getElementById('txtSaBlockIndex')?.value;
    const fileInput = document.getElementById('txtSaBlockFile');
    const replace = document.getElementById('txtSaBlockReplace')?.checked;

    if (!testId || !blockIndex) {
        showNotification('Не выбран тест/блок', 'error');
        return;
    }
    ...
    const progressDiv = document.getElementById('txtSaBlockUploadProgress');
    const progressBar = document.getElementById('txtSaBlockUploadProgressBar');
    const statusText = document.getElementById('txtSaBlockUploadStatus');
    if (progressDiv) progressDiv.style.display = 'block';
    if (progressBar) progressBar.style.width = '30%';
    if (statusText) statusText.textContent = 'Загрузка…';

    try {
        const response = await fetch(`${ADMIN_API_URL}/upload-txt-sa-block`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${currentAdminToken}` },
            body: formData
        });
...
async function openUsmleTxtMixedUploadModal() {
    try {
        const tests = await fetchUsmleTestsCompact();
        if (!tests.length) {
            showNotification('Сначала создайте тест USMLE', 'error');
        ...
        const select = document.getElementById('txtMixedTestId');
        const presetTestId = document.getElementById('usmleQuestionsTestFilter')?.value || '';
        if (select) {
            select.innerHTML = '<option value="">Выберите тест USMLE</option>' +
                tests.map(t => `<option value="${t.id}">${escapeAdminHtml(t.name)}</option>`).join('');
        ...
        cachedUploadPreviewTags = await fillUploadTagsSelect();
        adminQuestionUploadSource = 'usmle';
        const modal = document.getElementById('txtMixedUploadModal');
        if (modal) modal.style.display = 'block';
    } catch (error) {
        console.error('Ошибка открытия смешанной загрузки USMLE:', error);
        showNotification('Ошибка загрузки тестов USMLE', 'error');
    }
}
...
async function fillUniFcTopics(universityId, selectedId = '', selectId = 'uniFcTopicId') {
    ...
    const isTxtDefault = selectId === 'uniTxtFcTopicId';
    sel.innerHTML = isFilter
        ? '<option value="">Все предметы</option>'
        : (isTxtDefault
            ? '<option value="">Из заголовков TXT</option>'
...
async function loadUniFcTopicsAdmin() {
    ...
    try {
        ...
        const list = await response.json();
        if (!list.length) {
            box.innerHTML = '<span style="color:var(--text-muted); font-size:0.9rem;">Предметов пока нет — добавьте первый</span>';
            return;
        }
        ...
        box.innerHTML = '<span style="color:var(--danger-color);">Ошибка загрузки предметов</span>';
    }
}
...
function requireUniFcUniversity(requireTopic = false) {
    const universityId = document.getElementById('uniFcUniversityFilter')?.value;
    const topicId = document.getElementById('uniFcTopicFilter')?.value || '';
    if (!universityId) {
        showNotification('Сначала выберите университет', 'error');
    ...
    if (requireTopic && !topicId) {
        showNotification('Выберите или добавьте предмет карточек', 'error');
        return null;
    }
    return { universityId, topicId };
}

async function openAddUniFlashcardModal() {
    const sel = requireUniFcUniversity(false);
    if (!sel) return;
...
function setFlashcardImageModalMode(mode) {
    const isUni = mode === 'university';
    const programEl = document.getElementById('fcImgProgramType');
    if (programEl) programEl.value = isUni ? 'university' : 'usmle';
    const usmleMeta = document.getElementById('fcImgUsmleMeta');
    const usmleTags = document.getElementById('fcImgUsmleTagsWrap');
    const uniMeta = document.getElementById('fcImgUniMeta');
    if (usmleMeta) usmleMeta.style.display = isUni ? 'none' : 'grid';
    if (usmleTags) usmleTags.style.display = isUni ? 'none' : 'block';
    if (uniMeta) uniMeta.style.display = isUni ? 'block' : 'none';
    const step = document.getElementById('fcImgStepGroup');
    if (step) step.required = !isUni;
}

async function openAddUniFlashcardWithImagesModal() {
    const sel = requireUniFcUniversity(false);
    if (!sel) return;
    ...
    setFlashcardImageModalMode('university');
    ...
    await fillSelectFromUniversities('fcImgUniversityId', sel.universityId);
    await fillUniFcTopics(sel.universityId, sel.topicId || '', 'fcImgTopicId');
    document.getElementById('fcImgIsFree').checked = false;
    document.getElementById('flashcardImageModal').style.display = 'block';
}

async function openEditUniFlashcardWithImagesModal(card) {
    ...
    setFlashcardImageModalMode('university');
    document.getElementById('flashcardImageModalTitle').textContent = 'Редактировать карточку';
    resetFcImgSideUI('front');
    resetFcImgSideUI('back');
    ...
    fcImgPasteSide = 'front';
    fcImgForcePasteSide = null;
    await fillSelectFromUniversities('fcImgUniversityId', card.universityId || '');
    await fillUniFcTopics(card.universityId || '', card.topicId || '', 'fcImgTopicId');
    document.getElementById('fcImgIsFree').checked = !!card.isFree;
    document.getElementById('flashcardImageModal').style.display = 'block';
}
...
async function editUniFlashcardAdmin(id) {
    try {
        ...
        await openEditUniFlashcardWithImagesModal(card);
    } catch (e) {
...
async function saveUniFlashcardAdmin(e) {
    ...
    try {
        ...
        await loadUniFlashcardsAdmin();
        await loadUniFcTopicsAdmin();
        await fillUniFcTopics(payload.universityId, '', 'uniFcTopicFilter');
    } catch (err) {
        showNotification(err.message || 'Ошибка сохранения', 'error');
    }
}

async function openUniTxtFlashcardsUploadModal() {
    const sel = requireUniFcUniversity(false);
    if (!sel) return;
    try {
        await fillSelectFromUniversities('uniTxtFcUniversityId', sel.universityId);
        await fillUniFcTopics(sel.universityId, sel.topicId || '', 'uniTxtFcTopicId');
        document.getElementById('uniTxtFcIsFree').checked = false;
        const modal = document.getElementById('uniTxtFlashcardsUploadModal');
        if (modal) modal.style.display = 'block';
    } catch (e) {
        showNotification('Не удалось открыть загрузку TXT', 'error');
    }
}

async function handleUniTxtFlashcardsUpload(e) {
    e.preventDefault();
    const universityId = document.getElementById('uniTxtFcUniversityId')?.value || '';
    const topicId = document.getElementById('uniTxtFcTopicId')?.value || '';
    const isFree = !!document.getElementById('uniTxtFcIsFree')?.checked;
    const fileInput = document.getElementById('uniTxtFlashcardsFile');
    ...
    const formData = new FormData();
    formData.append('pdf', fileInput.files[0]);
    formData.append('programType', 'university');
    formData.append('universityId', universityId);
    if (topicId) formData.append('topicId', topicId);
    formData.append('isFree', isFree ? 'true' : 'false');

    const progressDiv = document.getElementById('uniTxtFlashcardsUploadProgress');
    const progressBar = document.getElementById('uniTxtFlashcardsUploadProgressBar');
    const statusText = document.getElementById('uniTxtFlashcardsUploadStatus');
    ...
    try {
        ...
        const result = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(result.error || 'Ошибка загрузки TXT');

        if (progressBar) progressBar.style.width = '100%';
        if (statusText) statusText.textContent = 'Готово!';
        setTimeout(async () => {
            showNotification(result.message || 'TXT загружен', 'success');
            const uniFilter = document.getElementById('uniFcUniversityFilter');
            if (uniFilter) uniFilter.value = String(universityId);
            document.getElementById('uniTxtFlashcardsUploadModal').style.display = 'none';
            document.getElementById('uniTxtFlashcardsUploadForm')?.reset();
            if (progressDiv) progressDiv.style.display = 'none';
            if (progressBar) progressBar.style.width = '0%';
            await loadUniFcTopicsAdmin();
            await fillUniFcTopics(universityId, topicId || '', 'uniFcTopicFilter');
            await loadUniFlashcardsAdmin();
        }, 400);
    } catch (error) {
        console.error(error);
...
async function openAddUsmleFlashcardWithImagesModal() {
    ...
    resetFcImgSideUI('front');
    resetFcImgSideUI('back');
    setFlashcardImageModalMode('usmle');
    document.getElementById('flashcardImageModalTitle').textContent = 'Карточка с картинами';
    const presetStep = document.getElementById('usmleFlashcardsStepFilter')?.value || 'step1';
...
async function saveFlashcardWithImagesAdmin(e) {
    ...
    const programType = document.getElementById('fcImgProgramType')?.value === 'university'
        ? 'university'
        : 'usmle';

    let payload;
    if (programType === 'university') {
        const universityId = document.getElementById('fcImgUniversityId')?.value || null;
        if (!universityId) {
            showNotification('Выберите университет', 'error');
            return;
        }
        payload = {
            ...
            universityId,
            subjectId: null,
            topicId: document.getElementById('fcImgTopicId')?.value || null,
...
async function handleTxtMixedUpload(e) {
    e.preventDefault();
    const testId = document.getElementById('txtMixedTestId')?.value;
    const fileInput = document.getElementById('txtMixedFile');
    ...
    const progressDiv = document.getElementById('txtMixedUploadProgress');
    const progressBar = document.getElementById('txtMixedUploadProgressBar');
    const statusText = document.getElementById('txtMixedUploadStatus');
    ...
    try {
        const response = await fetch(`${ADMIN_API_URL}/upload-txt-mixed`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${currentAdminToken}` },
            body: formData
        });
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                ...
                const modal = document.getElementById('txtMixedUploadModal');
                if (modal) modal.style.display = 'none';
                const form = document.getElementById('txtMixedUploadForm');
                if (form) form.reset();
                if (progressDiv) progressDiv.style.display = 'none';
                if (progressBar) progressBar.style.width = '0%';
...
window.openUsmleSaBlock = openUsmleSaBlock;
window.openUsmleSaBlockUpload = openUsmleSaBlockUpload;
window.clearUsmleSaBlock = clearUsmleSaBlock;
